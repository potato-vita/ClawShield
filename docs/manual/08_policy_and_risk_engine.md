...
- 支持 `fallback_rule / guardrails_runtime` 模式（当前默认 `fallback_rule`）。
...
### goal_mismatch_tool_call

条件：alignment 评估给出 deny，且调用与目标/步骤/影响不一致。
