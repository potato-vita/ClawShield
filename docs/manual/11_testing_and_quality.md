...
- round7: graph 与风险链（含 `goal_mismatch_tool_call`）。
- round8: report 结构完整性。
- round9: 错误协议稳定性。
...
```bash
python3 -m unittest tests.integration.test_roundz11_opencaw_callback
```

仅执行策略相关：

```bash
python3 -m unittest tests.unit.test_policy_engine tests.integration.test_round5_policy
```
