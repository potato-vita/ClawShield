...
3. 时间线与图谱摘要
4. 风险命中与最终结论
5. 目标一致性摘要（goal/alignment/impact/explainability）
...
- `goal_summary`: 任务目标契约摘要。
- `alignment_summary`: tool-call 一致性统计与最差调用。
- `impact_summary`: 敏感访问/外联/高影响统计。
- `explainability_trace`: 逐步解释链（step/tool/alignment/effect）。
- `conclusion`: 结论文本。
