# ClawShield Demo Runbook (Goal-Consistency Edition)
...
- Click `run scenario` on `goal_mismatch_external_upload`.
- Explain:
  - task goal is summary, but tool action is outbound upload
  - alignment pre-check gives deny
  - chain hit `goal_mismatch_tool_call`
...
4. 再触发一次 `goal_mismatch_external_upload`，确认 report 出现 `goal_mismatch_tool_call`。
