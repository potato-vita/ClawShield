...
class RoundEightReportTestCase(unittest.TestCase):
    def test_report_endpoint_returns_complete_report_view_model(self) -> None:
        with TestClient(app) as client:
            ...
            run_events_page = client.get(f"/api/v1/ui/runs/{run_id}/events")
            self.assertEqual(run_events_page.status_code, 200)
            report_page = client.get(f"/api/v1/ui/runs/{run_id}/report")
            self.assertEqual(report_page.status_code, 200)

            events_page_resp = client.get(
                "/api/v1/events",
                params={"run_id": run_id, "order": "desc", "limit": 20, "offset": 0},
            )
            self.assertEqual(events_page_resp.status_code, 200)
            events_page_payload = events_page_resp.json()["data"]
            self.assertIn("pagination", events_page_payload)
            pagination = events_page_payload["pagination"]
