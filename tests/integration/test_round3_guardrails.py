...
class RoundThreeGuardrailsTestCase(unittest.TestCase):
    def test_tool_call_semantic_decision_and_events(self) -> None:
        with TestClient(app) as client:
            system_status = client.get("/api/v1/system/status").json()
            self.assertTrue(system_status["success"])
            self.assertIn(system_status["data"]["guardrails_mode"], ["fallback_rule", "guardrails_runtime"])
