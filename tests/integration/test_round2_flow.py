...
class RoundTwoFlowTestCase(unittest.TestCase):
    ...
    def test_task_ingest_and_run_queries(self) -> None:
        with TestClient(app) as client:
            ...
            self.assertTrue(ingest_payload["data"]["goal_id"].startswith("goal_"))
            self.assertIn("task_intent", ingest_payload["data"])
            self.assertIn("allowed_action_types", ingest_payload["data"])
            ...
            goal_resp = client.get(f"/api/v1/runs/{run_id}/goal")
            self.assertEqual(goal_resp.status_code, 200)
            goal_payload = goal_resp.json()
            self.assertEqual(goal_payload["data"]["run_id"], run_id)
            self.assertTrue(goal_payload["data"]["goal_id"].startswith("goal_"))

            steps_resp = client.get(f"/api/v1/runs/{run_id}/steps")
            self.assertEqual(steps_resp.status_code, 200)
            steps_payload = steps_resp.json()
            self.assertEqual(steps_payload["data"]["run_id"], run_id)
            self.assertGreaterEqual(len(steps_payload["data"]["steps"]), 1)
            self.assertEqual(steps_payload["data"]["steps"][0]["state"], "analyzing")
            ...
            self.assertIn("task_step_created", event_types)
