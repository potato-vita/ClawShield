...
from app.schemas.audit import AuditReportPayload, GoalReportSummary
...
class ReportService:
    ...
    def get_report(self, db: Session, run_id: str) -> AuditReportPayload:
        ...
        payload = report_assembler.assemble(
            ...
            goal_summary=self._build_goal_summary(db=db, run_id=run_id),
        )
    ...
    @staticmethod
    def _build_goal_summary(db: Session, run_id: str) -> GoalReportSummary:
        goal = goal_repository.get_latest_by_run_id(db=db, run_id=run_id)
        if goal is None:
            return GoalReportSummary()
        return GoalReportSummary(
            task_intent=goal.task_intent,
            objective_summary=goal.objective_summary,
            allowed_actions=list(goal.allowed_action_types_json),
        )
