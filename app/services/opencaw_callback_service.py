...
class OpenClawCallbackService:
    ...
    def process_message_callback(self, db: Session, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for index, item in enumerate(items, start=1):
            ...
            before_step = self._task_state_service.ensure_active_step_for_tool_call(db=db, run_id=run_id)
            after_step = self._task_state_service.on_message(db=db, run_id=run_id, actor_type=actor_type, content=content)
            ...
            if after_step.step_id != before_step.step_id:
                self._audit_service.record_event(
                    ...
                    event_stage="conversation",
                    actor_type="system",
                    output_summary=f"from={before_step.state}; to={after_step.state}; step_id={after_step.step_id}",
                    status="recorded",
                )
            results.append(
                {
