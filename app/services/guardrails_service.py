...
class GuardrailsService:
    ...
    def evaluate_tool_call(self, db: Session, payload: ToolCallRequest) -> ToolCallDecision:
        ...
        mode = self._resolve_mode(self._load_config())
        if mode == "guardrails_runtime":
            # Runtime integration is optional. Keep deterministic fallback behavior for stability.
            evaluation = evaluate_candidate_action(
                task_type=context["task_type"],
                tool_id=payload.tool_id,
                dialog_state=context["dialog_state"],
            )
            runtime_note = "guardrails_runtime_requested_fallback_used"
        else:
            evaluation = evaluate_candidate_action(
                task_type=context["task_type"],
                tool_id=payload.tool_id,
