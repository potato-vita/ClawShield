    ...
    def load(self) -> PolicyRulesBundle:
        ...
        risk_chain_rules = [
            RiskChainRule(
                ...
                task_types=[str(v).lower() for v in item.get("task_types", [])] if isinstance(item.get("task_types", []), list) else [],
                path_templates=[str(v) for v in item.get("path_templates", [])] if isinstance(item.get("path_templates", []), list) else [],
