...
export interface CoreAuditEvent{tool_call_id:string;session_id:string;run_id:string;tool_name:string;tool_kind:string;resource_hint:string|null;risk_hint:string|null;status:string;started_at:string;decision:string|null;risk_level:string|null;matched_rules:string[]|null;reason:string|null}
...
export async function getAuditBundle(limit=200):Promise<{sessions:AuditSession[];runs:AuditRun[];toolCalls:ToolCall[]}>{
  ...
  const sessions=new Map<string,AuditSession>(sessionRows.map(row=>[row.session_id,{id:row.session_id,title:row.session_id,subtitle:sessionSubtitle(row),risk:risk(row.risk_level),time:time(row.last_seen_at),runIds:row.latest_run_id?[row.latest_run_id]:[]}]))
  const runs=new Map<string,AuditRun>(sessionRows.flatMap(row=>row.latest_run_id?[[row.latest_run_id,{id:row.latest_run_id,sessionId:row.session_id,title:`Run ${row.latest_run_id}`,startedAt:row.latest_run_started_at??row.first_seen_at,decision:decision(row.final_decision),risk:risk(row.risk_level),summary:row.tool_call_count?`${row.tool_call_count} tool calls · ${row.message_count} message events`:`Conversation-only run · ${row.message_count} message events`} as AuditRun] as const]:[]));
  const toolCalls=events.map<ToolCall>((event)=>{
    ...
    if(!current)sessions.set(event.session_id,{id:event.session_id,title:event.session_id,subtitle:`${event.tool_name} · ${event.resource_hint??event.tool_kind}`,risk:eventRisk,time:time(event.started_at),runIds:[event.run_id]});
    ...
    if(!runs.has(event.run_id))runs.set(event.run_id,{id:event.run_id,sessionId:event.session_id,title:`Run ${event.run_id}`,startedAt:event.started_at,decision:eventDecision,risk:eventRisk,summary:event.reason??"Core audit run"});
    return{id:event.tool_call_id,sessionId:event.session_id,runId:event.run_id,time:time(event.started_at),toolName:event.tool_name,toolKind:normalizeKind(event.tool_kind),resource:event.resource_hint??"—",decision:eventDecision,riskLevel:eventRisk,latencyMs:0,policyHits:event.matched_rules??[],argumentsSummary:event.reason??"Core event metadata",nodeId:`tool:${event.tool_call_id}`};
  });
  return{sessions:[...sessions.values()],runs:[...runs.values()],toolCalls};
}
function sessionSubtitle(row:CoreSession):string{if(row.tool_call_count>0)return `${row.tool_call_count} tool calls · ${row.message_count} messages`;return `${row.latest_event_type??"conversation"} · ${row.message_count} messages`;}
